...
def extract_status(subject, body):
    ...
    if any(kw in text for kw in ['QUOTE', 'QUOTATION']):
        return "报价"
    elif any(kw in text for kw in ['OFFER', 'AVAILABLE']):
        return "有货"
    elif any(kw in text for kw in ['REQUEST', 'RFQ', '需求', '需要', 'LOOKING FOR']):
        return "需求"
    elif any(kw in text for kw in ['ORDER', 'CONFIRM']):
        return "订单"
    else:
        return "其他"
...
def main():
    print("Blue Sky Technics 需求汇总（高效版）")
    print("=" * 50)
    ...
    try:
        ...
        # Only check the last 200 emails
        recent_ids = email_ids[-200:] if len(email_ids) > 200 else email_ids
        print(f"Checking last {len(recent_ids)} emails out of {len(email_ids)} total")
        ...
        for email_id in recent_ids:
            try:
                ...
                if 'domas@blueskytechnics.com' not in from_raw and 'blue sky technics' not in from_raw:
                    continue
                ...
                found_count += 1
                print(f"[{found_count}] {date_formatted} | {subject[:50]}")
                
            except Exception as e:
                continue
        
        print(f"\nFound {len(results)} Blue Sky Technics emails")
        ...
        if results:
            ...
            print("No Blue Sky Technics emails found in recent emails.")
