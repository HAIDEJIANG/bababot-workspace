...
def extract_quantity(text):
    try:
        if not text:
            return ""
        patterns = [r'Qty[:\s]*(\d+)', r'Quantity[:\s]*(\d+)', r'(\d+)\s*pcs?']
        for pattern in patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                return match.group(1)
...
def extract_status(subject, body):
    try:
        ...
        if any(kw in text for kw in ['QUOTE', 'QUOTATION', '报价']):
            return "报价"
        elif any(kw in text for kw in ['OFFER', 'AVAILABLE', '有货']):
            return "有货"
        elif any(kw in text for kw in ['REQUEST', 'RFQ', '需求', '需要', 'LOOKING FOR']):
            return "需求"
        elif any(kw in text for kw in ['ORDER', 'CONFIRM', '确认']):
            return "订单"
    except:
        pass
    return "其他"
...
def main():
    print("=" * 60)
    print("Blue Sky Technics 需求汇总（简化版）")
    print("=" * 60)
    ...
    try:
        ...
        print(f"Searching for: {SEARCH_FROM}")
        print()
        ...
        for idx, email_id in enumerate(email_ids):
            try:
                ...
                match = False
                for from_name in SEARCH_FROM:
                    if from_name.lower() in from_decoded.lower():
                        match = True
                        break
                ...
                print(f"[{len(results)}] {date_formatted} | {from_decoded[:40]} | {subject[:50]}")
        ...
        if results:
            ...
            print("Tips:")
            print("1. Check sender names are correct")
            print("2. Check if emails exist in inbox")
