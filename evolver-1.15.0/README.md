...
`src/evolve.js`, Behavior=Reads logs, selects genes, builds prompts, writes artifacts, Executes Shell Commands?=Read-only git/process queries only
`src/gep/prompt.js`, Behavior=Assembles the GEP protocol prompt string, Executes Shell Commands?=No (pure text generation)
`src/gep/selector.js`, Behavior=Scores and selects Genes/Capsules by signal matching, Executes Shell Commands?=No (pure logic)
`src/gep/solidify.js`, Behavior=Validates patches via Gene `validation` commands, Executes Shell Commands?=Yes (see below)
`index.js` (loop recovery), Behavior=Prints `sessions_spawn(...)` text to stdout on crash, Executes Shell Commands?=No (text output only; execution depends on host runtime)
