...
# -*- coding: utf-8 -*-
"""
LinkedIn 深度采集脚本 v3.0 - 稳定性增强版

修复内容：
1. 分批次采集 - 避免长时间运行超时（4 批次 × 15 分钟）
2. New Posts 自动刷新 - 重复率超过 50% 自动刷新
3. 浏览器会话管理 - 每 20 分钟重启一次
4. 智能重复度检测 - 动态调整阈值
5. 进度持久化 - 每项完成后立即保存
6. 错误恢复 - 超时自动重试
...
log_file = OUTPUT_DIR / f"linkedin_collection_log_v3_{run_id}.txt"
results_file = OUTPUT_DIR / f"linkedin_posts_batch_v3_{run_id}.csv"
state_file = OUTPUT_DIR / f"linkedin_collection_state_v3_{run_id}.json"
...
def browser_command(cmd, args=None, max_retries=3):
    """执行浏览器命令（带重试）"""
    last_error = None
    ...
    for attempt in range(max_retries):
        try:
            ...
            # result = browser(action=cmd, **args)
            # return result
            
            log(f"浏览器命令：{cmd} (尝试{attempt+1}/{max_retries})")
            return {'status': 'success', 'attempt': attempt + 1}
            
        except Exception as e:
            last_error = str(e)
            log(f"❌ 浏览器命令失败：{cmd} - {last_error}")
...
def browser_click_new_posts():
    ...
    for selector in selectors:
        ...
        if result.get('status') == 'success':
            browser_command('act', {'kind': 'click', 'selector': selector})
...
def browser_restart():
    ...
    time.sleep(5)
    browser_command('start', {'profile': 'openclaw'})
    time.sleep(10)
...
def browser_get_snapshot():
    """获取页面快照"""
    return browser_command('snapshot', {'refs': 'aria'})
