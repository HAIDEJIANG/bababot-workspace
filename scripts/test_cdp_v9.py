#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CDP 测试脚本 v9 - 使用 AttachToTarget 并等待事件"""
import websocket
import json
import time
import requests
...
session_id = result.get('params', {}).get('sessionId')
if not session_id:
...
if count > 0:
    ...
    (function() {
        ...
        return Array.from(articles).map(function(a, i) {
            return {
                index: i,
                text: a.innerText.substring(0, 500),
                author: (a.querySelector('[href*="/in/"]') || {}).innerText || 'Unknown'
            };
        });
    })()
    """
    result = send_to_session("Runtime.evaluate", {"expression": js, "returnByValue": True})
    posts = result.get('result', {}).get('value', [])
    print(f"\nFound {len(posts)} posts:")
    for post in posts[:5]:
        print(f"  [{post.get('index')}] {post.get('author', 'Unknown')[:30]}... ({post.get('text', '')[:100]}...)")
