...
## What It Does
**Extract**, When it runs=Between sessions, What it does=Pulls structured facts (decisions, preferences, learnings) from daily notes and transcripts
**Link**, When it runs=Between sessions, What it does=Builds a knowledge graph — connects related facts, flags contradictions
**Brief**, When it runs=Between sessions, What it does=Generates a compact `BRIEFING.md` loaded at session start
**Ingest**, When it runs=On demand, What it does=Imports external knowledge (ChatGPT exports, etc.) into searchable memory
**Performance Hooks**, When it runs=During sessions, What it does=Pre-game briefing injection, tool discipline, output compression, after-action review
...
## Output Files
`BRIEFING.md`, Location=Workspace root, Purpose=Daily context cheat sheet
`extracted.jsonl`, Location=`memory/`, Purpose=All extracted facts (append-only)
`knowledge-graph.json`, Location=`memory/`, Purpose=Full graph with embeddings and links
`knowledge-summary.md`, Location=`memory/`, Purpose=Human-readable graph summary
`knowledge/chatgpt/*.md`, Location=`memory/`, Purpose=Ingested ChatGPT conversations
