...
- `.git/` (git internals), `node_modules/` (if any), Files in `.gitignore`
...
CLI Installed, Status= Done, Command=`which clawdhub`
GitHub Repo, Status= Done, Command=https://github.com/brokemac79/webchat-audio-notifications
ClawdHub Login, Status=⏳ Needed, Command=`clawdhub login`
Publish, Status=⏳ Ready, Command=`clawdhub publish . --slug webchat-audio-notifications --name "Webchat Audio Notifications" --version 1.0.0 --changelog "Initial release"`
