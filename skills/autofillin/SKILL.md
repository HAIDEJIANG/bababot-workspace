...
## Error Handling
Unsafe browser, Cause=Google OAuth blocked, Solution=Use Playwright instead of Chrome debug
Login required, Cause=Session expired, Solution=Run with --save-storage
Element not found, Cause=Page changed, Solution=Take new snapshot
Upload failed, Cause=Wrong file type, Solution=Check webkitdirectory
Port in use, Cause=Another Chrome instance, Solution=Script auto-kills conflicting process
Chrome not found, Cause=Not installed, Solution=Run setup-env.sh
