...
## vs bird CLI
설치, bird CLI=brew 필요, chirp (browser)=Xvfb만 있으면 됨
인증, bird CLI=쿠키 추출, chirp (browser)=브라우저 세션
안정성, bird CLI=API 기반, chirp (browser)=UI 의존 (변경 가능)
속도, bird CLI=빠름, chirp (browser)=약간 느림