...
List sessions, tmux=`list-sessions`, zellij=`list-sessions`
Create session, tmux=`new-session -d`, zellij=`new-session --detach`
Attach, tmux=`attach -t`, zellij=`attach --session`
Send keys, tmux=`send-keys`, zellij=`action write-chars`
Capture pane, tmux=`capture-pane`, zellij=`pipe`
Kill session, tmux=`kill-session`, zellij=`delete-session`
Detach, tmux=`Ctrl+b d`, zellij=`Ctrl+p d`
