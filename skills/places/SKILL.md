...
## Places vs Query
Method, `/places`=POST, `/query`=GET
Input, `/places`=Free-form OR structured address, `/query`=Natural language with context
Coordinates, `/places`=Returns them (geocoding), `/query`=Can auto-generate for search center
AI Ranking, `/places`=No, `/query`=Yes
Photos, `/places`=Optional street-level imagery, `/query`=No
Best For, `/places`="Eiffel Tower", address lookup, `/query`="quiet cafes near Times Square"
