...
LCP, Insight Name=`LCPBreakdown`, What to Look For=Time to largest contentful paint; breakdown of TTFB, resource load, render delay
CLS, Insight Name=`CLSCulprits`, What to Look For=Elements causing layout shifts (images without dimensions, injected content, font swaps)
Render Blocking, Insight Name=`RenderBlocking`, What to Look For=CSS/JS blocking first paint
Document Latency, Insight Name=`DocumentLatency`, What to Look For=Server response time issues
Network Dependencies, Insight Name=`NetworkRequestsDepGraph`, What to Look For=Request chains delaying critical resources
...
- Webpack: `webpack.config.js`, `webpack.*.js`, Vite: `vite.config.js`, `vite.config.ts`, Rollup: `rollup.config.js`, `rollup.config.mjs`
- esbuild: `esbuild.config.js`, build scripts with `esbuild`
- Parcel: `.parcelrc`, `package.json` (parcel field)
- Next.js: `next.config.js`, `next.config.mjs`, SvelteKit: `svelte.config.js`, Astro: `astro.config.mjs`
