...
`CLAWSHELL_PUSHOVER_USER`, Default=—, Description=Pushover user key
`CLAWSHELL_PUSHOVER_TOKEN`, Default=—, Description=Pushover app token
`CLAWSHELL_TELEGRAM_BOT_TOKEN`, Default=—, Description=Telegram bot token (alternative)
`CLAWSHELL_TELEGRAM_CHAT_ID`, Default=—, Description=Telegram chat ID (alternative)
`CLAWSHELL_TIMEOUT_SECONDS`, Default=300, Description=Seconds to wait for approval before auto-reject
`CLAWSHELL_LOG_DIR`, Default=logs/, Description=Directory for JSONL log files
`CLAWSHELL_LOG_LEVEL`, Default=info, Description=Log verbosity: debug, info, warn, error
`CLAWSHELL_BLOCKLIST`, Default=—, Description=Comma-separated extra blocked commands
`CLAWSHELL_ALLOWLIST`, Default=—, Description=Comma-separated extra allowed commands
