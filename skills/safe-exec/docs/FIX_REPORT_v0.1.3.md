...
1. **Plugin Version** (`~/.openclaw/extensions/safe-exec/`)
 - TypeScript implementation, Registered in `plugins.entries.safe-exec`, Incorrect architecture

2. **Skill Version** (`~/.openclaw/skills/safe-exec/`)
 - Bash script implementation, Loaded from `skills.load.extraDirs`, Correct architecture

### Why This Was Wrong
**Purpose**, Plugin=Extend OpenClaw core, Skill=Tools for Agents
**Example**, Plugin=Feishu channel, Skill=Web search, TTS
**Language**, Plugin=TypeScript, Skill=Any (bash, py, etc.)
**Location**, Plugin=`extensions/`, Skill=`skills/`
**Config**, Plugin=`plugins.entries`, Skill=`skills.load.extraDirs`
