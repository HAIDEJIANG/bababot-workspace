...
- 帮你管理文件, 自动化系统维护, 处理重复性任务, 甚至写代码,部署服务

但它们也可能:

- 一句话删光你的数据, 破坏整个系统, 打开安全漏洞, 泄露敏感信息
 ...
 **CRITICAL**, 检测模式=`rm -rf /`, 真实案例=删除系统根目录
 **CRITICAL**, 检测模式=`dd if=`, 真实案例=磁盘破坏命令
 **CRITICAL**, 检测模式=`mkfs.*`, 真实案例=格式化文件系统
 **CRITICAL**, 检测模式=`:(){:\, 真实案例=:&};:`, Fork 炸弹 DoS 攻击
🟠 **HIGH**, 检测模式=`chmod 777`, 真实案例=设置全局可写权限
🟠 **HIGH**, 检测模式=`curl \, 真实案例=bash`, 管道下载到 shell
🟠 **HIGH**, 检测模式=写入 `/etc/`, 真实案例=篡改系统配置
🟡 **MEDIUM**, 检测模式=`sudo`, 真实案例=特权操作
🟡 **MEDIUM**, 检测模式=防火墙修改, 真实案例=网络暴露风险
...
### 双语文档
- 完整的英文 README, 语言切换链接, 国际化友好

### 增强的文档
- 详细的 OpenClaw 集成指南, 真实使用场景示例, 最佳实践指南, 完整的 FAQ

### 新增工具
- `safe-exec-add-rule` - 快速管理规则, 导入/导出规则集, 批量操作支持

### 改进的用户体验
- 彩色输出和表情符号, 更清晰的风险提示, 快捷批准命令
...
### v0.2.0 (已完成)
- [x] 规则快速管理工具, [x] 完整的双语文档, [x] OpenClaw 深度集成, [x] 审计日志系统
...
- **GitHub**: [给个 Star](https://github.com/OTTTTTO/safe-exec), **Discord**: [OpenClaw Community](https://discord.gg/clawd), **Email**: otto@example.com, **Twitter**: @yourhandle

## 学习资源
- [完整文档](https://github.com/OTTTTTO/safe-exec#readme), [视频教程](https://youtube.com/)(即将推出), [API 文档](https://github.com/OTTTTTO/safe-exec/blob/main/docs/API.md)
