 ...
 **CRITICAL**, 检测模式=`rm -rf /`, 示例=删除系统文件
 **CRITICAL**, 检测模式=`dd if=`, 示例=磁盘破坏
 **CRITICAL**, 检测模式=`mkfs.*`, 示例=格式化文件系统
 **CRITICAL**, 检测模式=Fork bomb, 示例=系统 DoS
🟠 **HIGH**, 检测模式=`chmod 777`, 示例=权限提升
🟠 **HIGH**, 检测模式=`curl \, 示例=bash`, 代码注入
🟠 **HIGH**, 检测模式=写入 `/etc/`, 示例=系统配置篡改
🟡 **MEDIUM**, 检测模式=`sudo`, 示例=特权操作
🟡 **MEDIUM**, 检测模式=防火墙修改, 示例=网络暴露
